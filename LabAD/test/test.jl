...
function check_case(X, y, w, loss_function, activation, Reference_diff, Given_diff, rtol=1e-8,hessian=false, broken=false)
    ...
    try
        ...
        if broken 
            @test_skip isapprox(norm(∇f - ∇r), 0.0; atol=rtol*norm(∇f))
        else
...
function run_gradient_tests(Reference_diff, Given_diff, loss_function, activation; 
                            ...
                            num_repeats=5,
                            rtol=1e-8, hessian=false, broken=false) #fail for 1e-15
    @testset "$name" begin
        ...
        for _ in 1:num_repeats
            ...
            check_case(X, y, w, loss_function, activation, Reference_diff, Given_diff, rtol, hessian)
    ...
    @testset "$name with sparse weights" begin # Fail for IfElseReverse
        ...
        for _ in 1:num_repeats
            ...
            check_case(X, y, w, loss_function, activation, Reference_diff, Given_diff, rtol, hessian, broken)
        end
    end
...
function run_gradient_tests(Reference_diff, Given_diff; kws...)
    ...
    # 3) ReLU
    run_gradient_tests(Reference_diff, Given_diff, mse, relu_activation, name="ReLU"; kws...)

    # 4) Softmax
    run_gradient_tests(Reference_diff, Given_diff, mse, relu_activation, name="Softmax"; classification = true, kws...)
    return
end
