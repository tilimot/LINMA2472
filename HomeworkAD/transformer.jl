text = read("shakespeare (1).txt", String)   # ou autre
text = lowercase(text)
...
Génère l'encodage positionnel (PE) sinusoïdal.
Retourne une matrice (d_model × T).
"""
...
function train_attention_model()
    ...
    function attention_loss(W_params)
        ...
        # 1. Stabilité numérique (conservation du clipping)
        probs = softmax(logits)
        log_probs = log.(probs .+ 1f-8) 
        
        # 2. Simplification du gradient : 
        # Le produit .* y_onehot isole le log(probabilité) de la classe cible
        log_target_prob = y_onehot .* log_probs 
        
        # On veut la somme des pertes négatives: - sum(log(p_target))
        # Note : Julia somme sur toutes les dimensions par défaut.
...
"""
function generate_text(W_trained, config, prompt=""; max_new_tokens=100, temperature=0.8)
