...
function Base.:*(A::Flatten{<:VectNode}, X::AbstractMatrix)
    # On multiplie chaque composant de A.value (donc chaque matrice contenue dans les VectNode)
    # par X, et on renvoie un Flatten de ces résultats.
...
# Reduction and scalar ops that preserve the computation graph
import Base: sum
function Base.sum(v::VectNode)
	s = sum(v.value)
	# derivative of sum w.r.t each element is 1, so backprop returns Δ for each element
...
function softmax(x::VectNode)
	...
	s = exps ./ sums
	# Backprop: for each row, given incoming Δ (n,k), gradient wrt logits is
	# s .* (Δ .- sum(Δ .* s, dims=2))
	return VectNode(s, zero(s), [(x, Δ -> s .* (Δ .- sum(Δ .* s, dims=2)))])
end


# Make VectNode behave like the underlying value for array-like operations
# Delegate explicitly to Base.iterate to avoid calling a local `iterate` function
