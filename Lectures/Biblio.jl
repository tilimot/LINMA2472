...
function load!()
    file = joinpath(@__DIR__, "biblio.bib")
...
function citation_key(d; max_authors = 4)
    return join([first_letter(s.last) for s in d.authors[1:min(length(d.authors), max_authors)]]) * d.date.year[end-1:end]
end
...
function cite(d, what)
    return "[" * citation_key(d) * "; " * what * "]"
end

# Inspired from `Bibliography.xnames` but deals with too long list of authors
function names(d; max_authors = 4)
    entry_names = d.authors[1:min(length(d.authors), max_authors)]
    ...
    if length(d.authors) > max_authors
        str *= ", et al."
    end
    return str
end

function print_entry(io::IO, d; links = true, venue = true, doi = true, cite = true)
    if cite
        print(io, Biblio.cite(d))
        print(io, ' ')
    end
    print(io, _fix(names(d)))
    print(io, ". ")
    title = _fix(d.title)
